...
class Z3ExprWrapper:
    ...
    def from_expr(cls, expr: z3.ExprRef) -> "Z3ExprWrapper":
        ...
        return cls(
            expr_str=str(expr), expr_type=str(type(expr)), expr_sort=str(expr.sort())
        )
...
class MemoryAccess:
    ...
    @classmethod
    def create(
        cls, ctx: z3.Context, addr_name: str, val_name: str, is_read: bool
    ) -> "MemoryAccess":
...
class EVMSymbolicExecutor:
    ...
    def create_initial_state(self) -> SymbolicState:
        ...
        state.variables["contract_balance_usdc"] = Z3ExprWrapper.from_expr(
            contract_usdc
        )
    ...
    def execute(self) -> List[SymbolicState]:
        ...
        while work_list and len(self.states) < MAX_STATES:
            ...
            if current_state.depth >= MAX_DEPTH:
                logger.warning(
                    f"Reached maximum depth {MAX_DEPTH} at PC {current_state.pc}"
                )
                continue
    ...
    def _op_mload(self, state: SymbolicState) -> List[SymbolicState]:
        ...
        successor.memory_accesses.append(
            MemoryAccess.create(
                self.ctx, f"addr_{state.depth}", f"val_{state.depth}", True
            )
        )
    ...
    def _op_mstore(self, state: SymbolicState) -> List[SymbolicState]:
        ...
        successor.memory_accesses.append(
            MemoryAccess.create(
                self.ctx, f"addr_{state.depth}", f"val_{state.depth}", False
            )
        )
    ...
    def _handle_push(self, state: SymbolicState, size: int) -> List[SymbolicState]:
        ...
        successor.stack.append(
            Z3ExprWrapper.from_expr(self.ctx.bv_val(push_value, 256))
        )
    ...
    def _handle_swap(self, state: SymbolicState, position: int) -> List[SymbolicState]:
        ...
        successor.stack[-1], successor.stack[-(position + 1)] = (
            successor.stack[-(position + 1)],
            successor.stack[-1],
        )
...
class BLSSSBuilder:
    ...
    def build_from_states(self, states: List[SymbolicState]) -> BLSSSStructure:
        ...
        return BLSSSStructure(metadata_array=self.metadata, symbolic_states=self.states)


def analyze_bytecode(
    bytecode: Union[str, bytes], output_format: str, output_file: str
) -> None:
...
def export_to_diagram(data: BLSSSStructure, filename: str) -> None:
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            f"  Entries Per Cache Line: {data.metadata_array.cache_line_size // 8}\n\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze EVM bytecode using symbolic execution and BLSSS"
    )
    ...
    parser.add_argument(
        "--format",
        choices=["json", "yaml", "diagram"],
        default="json",
        help="Output format (default: json)",
    )
    parser.add_argument(
        "--output",
        default="blsss_analysis.json",
