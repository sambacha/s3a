...
class SymbolicState:
    ...
    memory: z3.ArrayRef = dataclasses.field(default_factory=lambda: z3.K(BV256, z3.BitVecVal(0, 8))) 
    storage: z3.ArrayRef = dataclasses.field(default_factory=lambda: z3.K(BV256, z3.BitVecVal(0, 256)))
    path_condition: z3.BoolRef = dataclasses.field(default_factory=lambda: z3.BoolVal(True))
    gas_used: z3.BitVecRef = dataclasses.field(default_factory=lambda: z3.BitVecVal(0, 256))

    def __repr__(self) -> str:
        # Simplified representation for brevity
        return (f"SymbolicState(stack_depth={len(self.stack)}, "
...
def annotate_block(
    ...
    current_state = dataclasses.replace(initial_state, # Pass the object to replace first
