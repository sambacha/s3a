...
class TraceCollector:
    ...
    def get_trace(self, tx_hash: str, tracer_type: str = "callTracer", tracer_config: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "error" in trace:
                logger.error("Error received from debug_traceTransaction", tx_hash=tx_hash, error=trace["error"], tracer=tracer_type)
                raise Exception(f"Trace error for {tx_hash} using {tracer_type}: {trace['error']}")
            ...
            logger.debug("Successfully collected trace", tx_hash=tx_hash, tracer=tracer_type)
            return trace["result"]

        except Exception as e:
            logger.exception("Error during trace collection", tx_hash=tx_hash, tracer=tracer_type, error=str(e))
...
if __name__ == '__main__':
    ...
    try:
        ...
        call_trace = collector.get_trace(test_tx_hash, tracer_type="callTracer", tracer_config={"withLog": True})
        print("Call Trace:")
