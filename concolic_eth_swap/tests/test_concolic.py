...
@pytest.fixture
def mock_concrete_executor(
    eth_usdc_swap_tx, eth_usdc_swap_trace, non_swap_tx, non_swap_trace
):
    ...
    def mock_trace(tx_hash):
        if tx_hash == "0xSWAP_HASH":
            return eth_usdc_swap_trace
        if tx_hash == "0xNON_SWAP_HASH":
            return non_swap_trace
    ...
    mock.get_contract_code.return_value = (
        b"\x60\x80\x60\x40\x52..."  # Placeholder bytecode
    )
...
def mock_symbolic_executor():
    ...
    def mock_execute(
        tx, block_context, contract_code, contract_address, max_paths, max_depth
    ):
...
def test_concolic_analysis_swap_detected(
    MockConcreteExecutor,
    MockSymbolicExecutor,
    mock_concrete_executor,
    mock_symbolic_executor,
    eth_usdc_swap_tx,
):
    ...
    with patch.object(
        ConcolicExecutor,
        "_check_swap_balance_pattern",
        side_effect=mock_check_swap_pattern,
    ):
        executor = ConcolicExecutor("http://mock-provider:8545")
...
@patch("concolic_eth_swap.core.concolic.SymbolicExecutor")
@patch("concolic_eth_swap.core.concolic.ConcreteExecutor")
def test_concolic_analysis_no_swap(
    MockConcreteExecutor,
    MockSymbolicExecutor,
    mock_concrete_executor,
    mock_symbolic_executor,
    ...
    with patch.object(
        ConcolicExecutor, "_check_swap_balance_pattern", side_effect=mock_check_no_swap
    ):
        executor = ConcolicExecutor("http://mock-provider:8545")
        executor.concrete_executor = mock_concrete_executor
        executor.symbolic_executor = mock_symbolic_executor
        ...
        mock_concrete_executor.get_transaction_details.assert_called_with(
            "0xNON_SWAP_HASH"
        )
