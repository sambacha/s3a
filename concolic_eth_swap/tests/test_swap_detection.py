...
from ..swap_detection.patterns import SwapMethodSignature # Import if needed for mocking
...
@pytest.fixture
def mock_web3_for_analyzer(eth_usdc_swap_tx, swap_tx_receipt, non_swap_tx, non_swap_tx_receipt):
    ...
    def mock_get_receipt(tx_hash):
        if tx_hash == "0xSWAP_HASH": return swap_tx_receipt
        if tx_hash == "0xNON_SWAP_HASH": return non_swap_tx_receipt
        return None
...
@patch('concolic_eth_swap.swap_detection.analyzer.ConcolicExecutor')
@patch('concolic_eth_swap.swap_detection.analyzer.Web3')
def test_detect_swap_quick_signature(MockWeb3, MockConcolicExecutor, mock_web3_for_analyzer, eth_usdc_swap_tx):
    ...
    analyzer._extract_swap_path = MagicMock(return_value=["0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2", "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48"])

    result = analyzer.detect_eth_usdc_swap("0xSWAP_HASH", use_concolic=False) # Disable concolic
...
@patch('concolic_eth_swap.swap_detection.analyzer.ConcolicExecutor')
@patch('concolic_eth_swap.swap_detection.analyzer.Web3')
def test_detect_swap_quick_logs(MockWeb3, MockConcolicExecutor, mock_web3_for_analyzer, swap_tx_receipt):
    """Test detection via event logs"""
    # Modify the swap TX mock so signature check fails, forcing log check
    mock_swap_tx_no_sig = mock_web3_for_analyzer.eth.get_transaction("0xSWAP_HASH").copy()
    mock_swap_tx_no_sig['input'] = "0xDEADBEEF" # Invalid method ID
    mock_web3_for_analyzer.eth.get_transaction.side_effect = lambda h: mock_swap_tx_no_sig if h == "0xSWAP_HASH" else None
    ...
    with patch('concolic_eth_swap.swap_detection.analyzer.is_known_eth_usdc_pool', return_value=True):
...
@patch('concolic_eth_swap.swap_detection.analyzer.ConcolicExecutor')
@patch('concolic_eth_swap.swap_detection.analyzer.Web3')
def test_detect_swap_concolic(MockWeb3, MockConcolicExecutor, mock_web3_for_analyzer, non_swap_tx, non_swap_tx_receipt): # Added non_swap_tx fixture
    """Test detection falls back to concolic analysis"""
    # Use non_swap mocks for tx/receipt so quick checks fail
    mock_web3_for_analyzer.eth.get_transaction.side_effect = lambda h: non_swap_tx if h == "0xCONCOLIC_TEST" else None
    mock_web3_for_analyzer.eth.get_transaction_receipt.side_effect = lambda h: non_swap_tx_receipt if h == "0xCONCOLIC_TEST" else None
    ...
    mock_concolic.analyze_transaction.return_value = {
        ...
        "is_swap": True,
        "swap_details": {"swap_type": "USDC_TO_ETH", "details": {"usdc_spent": "5000000", "eth_received": "2000000000000000"}}
    }
...
@patch('concolic_eth_swap.swap_detection.analyzer.ConcolicExecutor')
@patch('concolic_eth_swap.swap_detection.analyzer.Web3')
def test_detect_no_swap(MockWeb3, MockConcolicExecutor, mock_web3_for_analyzer, non_swap_tx, non_swap_tx_receipt):
    """Test non-swap transaction is correctly identified"""
    mock_web3_for_analyzer.eth.get_transaction.side_effect = lambda h: non_swap_tx if h == "0xNON_SWAP_HASH" else None
    mock_web3_for_analyzer.eth.get_transaction_receipt.side_effect = lambda h: non_swap_tx_receipt if h == "0xNON_SWAP_HASH" else None

    MockWeb3.return_value = mock_web3_for_analyzer
    mock_concolic = MockConcolicExecutor.return_value
    # Configure mock concolic executor to return negative result
    mock_concolic.analyze_transaction.return_value = {
        "tx_hash": "0xNON_SWAP_HASH", "analysis_complete": True, "is_swap": False, "swap_details": {}
    }
