...
def test_symbolic_state_clone():
    ...
    state.path_constraints.append(PathConstraint(SymbolicValue(type=SymbolicType.CONCRETE, concrete_value=1, size=1)))

    clone = state.clone()

    # Check independence
    clone.stack.append(SymbolicValue(type=SymbolicType.CONCRETE, concrete_value=2))
    clone.storage["0x123"][1] = SymbolicValue(type=SymbolicType.CONCRETE, concrete_value=99)
    clone.path_constraints.append(PathConstraint(SymbolicValue(type=SymbolicType.CONCRETE, concrete_value=0, size=1)))
