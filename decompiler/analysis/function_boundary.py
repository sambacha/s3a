...
def match_call_sites(returns, jumps, jump_classifications):
    ...
    calls = [
        j
        for offset, j in jumps.items()
        if jump_classifications.get(offset) == "private-call"
    ]
...
def infer_function_boundaries(
    jumps, blocks, jump_classifications, stack_analysis
):  # Added jump_classifications
    """
    Four-step process:
    ...
    returns = find_return_instructions(
        jumps, jump_classifications
    )  # Pass classifications

    # Match with call sites
    call_return_pairs = match_call_sites(
        returns, jumps, jump_classifications
    )  # Pass classifications
