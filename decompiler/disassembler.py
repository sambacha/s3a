from pyevmasm import (
    disassemble_all,
    Instruction as PyevmInstruction,
)  # Use disassemble_all
...
def identify_basic_blocks(bytecode):
    ...
    try:
        ...
        print(
            f"[Debug] Raw instructions (disassemble_all): {raw_instructions}",
            file=sys.stderr,
        )  # Debug
    ...
    for pyevm_instr in raw_instructions:
        ...
        if isinstance(pyevm_instr, PyevmInstruction):
            ...
            custom_instr = Instruction(
                ...
                operands=[pyevm_instr.operand]
                if pyevm_instr.operand is not None
                else [],
            )
            ...
            print(
                f"[Debug] Unexpected type in raw_instructions: {type(pyevm_instr)}",
                file=sys.stderr,
            )  # Debug
    ...
    if not instructions:
        ...
        return {}
    print(
        f"[Debug] Custom instructions created: {len(instructions)}", file=sys.stderr
    )  # Debug
    ...
    instruction_offsets = sorted(
        offset_to_instruction.keys()
    )  # Get sorted list of valid instruction offsets
    print(
        f"[Debug] Instruction offsets: {instruction_offsets}", file=sys.stderr
    )  # Debug
    ...
    print(
        f"[Debug] Sorted block starts: {sorted_block_starts}", file=sys.stderr
    )  # Debug
    ...
    for i, start_offset in enumerate(sorted_block_starts):
        ...
        next_block_start_offset = (
            sorted_block_starts[i + 1]
            if i + 1 < num_starts
            else (instruction_offsets[-1] + 1 if instruction_offsets else 0)
        )
        print(
            f"[Debug] Block {i}: start={start_offset}, next_start={next_block_start_offset}",
            file=sys.stderr,
        )  # Debug
        ...
        print(
            f"[Debug] Block {i}: Filtered instructions count: {len(block_instructions)}",
            file=sys.stderr,
        )  # Debug
        ...
        if block_instructions:
            ...
            print(
                f"[Debug] Block {i}: Created block at offset {start_offset}",
                file=sys.stderr,
            )  # Debug
        else:
            print(
                f"[Debug] Block {i}: No instructions found for block starting at {start_offset}",
                file=sys.stderr,
            )  # Debug

    # 4. Identify Successors
    print(
        f"[Debug] Identifying successors for {len(basic_blocks)} blocks",
        file=sys.stderr,
    )  # Debug
    for start_offset, block in basic_blocks.items():
        ...
        if raw_last_instr and last_instr.opcode not in (
            "JUMP",
            "STOP",
            "RETURN",
            "REVERT",
            ...
            fallthrough_offset = last_instr.offset + get_instruction_size(
                raw_last_instr
            )
        ...
        if last_instr.opcode == "JUMPI":
            fallthrough_offset_jumpi = (
                last_instr.offset + get_instruction_size(raw_last_instr)
                if raw_last_instr
                else -1
            )
            if (
                fallthrough_offset_jumpi != -1
                and fallthrough_offset_jumpi in basic_blocks
            ):
            ...
            if prev_instr.opcode.startswith("PUSH") and prev_instr.operands:
                ...
                if isinstance(target_offset, int) and target_offset in basic_blocks:
                    ...
                    if (
                        ...
                        and target_block.instructions[0].opcode == "JUMPDEST"
                    ):
    ...
    print(
        f"[Debug] identify_basic_blocks returning {len(basic_blocks)} blocks",
        file=sys.stderr,
    )  # Debug
    return basic_blocks
