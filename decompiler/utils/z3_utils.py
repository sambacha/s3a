...
def setup_jump_constraints(jumps, blocks, stack_analysis):
    ...
    for jump_id, jump in jumps.items():
        ...
        if jump.opcode == 'JUMPI':
            ...
            solver.add(Implies(
                Not(vars['locally_resolved']),
                vars['is_private_return']
            ))
        
        # Each jump must have exactly one classification
        solver.add(Or(
            And(vars['is_intra_proc'], Not(vars['is_private_call']), Not(vars['is_private_return'])),
            And(Not(vars['is_intra_proc']), vars['is_private_call'], Not(vars['is_private_return'])),
            And(Not(vars['is_intra_proc']), Not(vars['is_private_call']), vars['is_private_return'])
        ))
