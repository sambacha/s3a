...
class EnhancedStorageAnalyzer(StorageAnalyzer):
    ...
    def analyze(self, bytecode: str) -> StorageLayout:
        ...
        logger.info(f"Storage layout analysis completed in {self.analysis_time:.2f} seconds")
        
        # Log type inference statistics
        stats = self.type_inference.get_inference_stats()
        logger.info(f"Type inference enhanced {stats['enhancements']} variables and resolved {stats['conflicts']} conflicts")
