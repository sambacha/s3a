...
try:
    ...
    logger.warning(
        "evmole package not installed. Enhanced analysis features will be limited."
    )
...
class EvmoleWrapper:
    ...
    def get_storage_layout(self, bytecode: Union[bytes, str]) -> List[Dict[str, Any]]:
        ...
        for record in analysis.storage:
            storage_item = {
                ...
                "type": record.type,
                "accessing_functions": {"reads": record.reads, "writes": record.writes},
            }
    ...
    def get_control_flow_data(self, bytecode: Union[bytes, str]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Extracted {len(blocks_by_start)} blocks and {len(function_blocks)} functions from control flow graph"
        )
        return {"blocks": blocks_by_start, "function_blocks": function_blocks}
    ...
    def _find_blocks_for_function(
        self, offset: int, blocks_by_start: Dict[int, Dict]
    ) -> List[int]:
        """
        ...
        if entry_block:
            ...
            potential_end = (
                entry_block["end"] + 500
            )  # Arbitrarily assume function size < 500 bytes
