...
class HybridTypeInference:
    ...
    def analyze_with_evmole(self, bytecode: str) -> None:
        ...
        logger.info(
            f"Evmole provided type information for {len(storage_layout)} storage slots"
        )
    ...
    def infer_variable_type(
        self, accesses: List[StorageAccess], original_inference: str
    ) -> Tuple[str, float]:
        """
        ...
        if slot_str in self.evmole_storage_info:
            ...
            accessing_functions = self.evmole_storage_info[slot_str][
                "accessing_functions"
            ]
            if accessing_functions["reads"] or accessing_functions["writes"]:
                evmole_confidence += (
                    0.1  # Boost confidence if we know which functions access it
                )

            logger.debug(
                f"Evmole suggests type {evmole_type} for slot {slot_str} with confidence {evmole_confidence:.2f}"
            )

        # Calculate confidence for original inference based on the evidence
        original_confidence = self._calculate_original_confidence(
            accesses, original_inference
        )
        logger.debug(
            f"Original inference suggests type {original_inference} for slot {slot_str} with confidence {original_confidence:.2f}"
        )
        ...
        if evmole_type and original_inference:
            if self._types_compatible(evmole_type, original_inference):
                ...
                specific_type = self._get_more_specific_type(
                    evmole_type, original_inference
                )
                combined_confidence = min(1.0, evmole_confidence + original_confidence)
                logger.info(
                    f"Combining compatible types {evmole_type} and {original_inference} -> {specific_type} with confidence {combined_confidence:.2f}"
                )
                ...
                if evmole_confidence > original_confidence:
                    logger.info(
                        f"Type conflict resolved in favor of evmole: {evmole_type} (confidence: {evmole_confidence:.2f}) vs {original_inference} (confidence: {original_confidence:.2f})"
                    )
                    return evmole_type, evmole_confidence
                else:
                    logger.info(
                        f"Type conflict resolved in favor of original inference: {original_inference} (confidence: {original_confidence:.2f}) vs {evmole_type} (confidence: {evmole_confidence:.2f})"
                    )
                    return original_inference, original_confidence
        elif evmole_type:
            # Only have evmole type
            logger.info(
                f"Using evmole type only: {evmole_type} (confidence: {evmole_confidence:.2f})"
            )
            return evmole_type, evmole_confidence
        else:
            # Fall back to original inference
            logger.info(
                f"No evmole data, using original inference: {original_inference} (confidence: {original_confidence:.2f})"
            )
            return original_inference, original_confidence

    def _calculate_original_confidence(
        self, accesses: List[StorageAccess], inferred_type: str
    ) -> float:
        """
        ...
        if concrete_values:
            ...
            if inferred_type == "address" and all(
                v < (1 << 160) for v in concrete_values
            ):
                confidence += 0.2
            ...
            if (
                len(set(concrete_values)) < len(concrete_values) / 2
    ...
    def _types_compatible(self, type1: str, type2: str) -> bool:
        ...
        if (type1 == "address" and type2 == "uint160") or (
            type1 == "uint160" and type2 == "address"
        ):
            return True

        # Special case: bytes and string are compatible (both dynamic)
        if (type1 == "bytes" and type2 == "string") or (
            type1 == "string" and type2 == "bytes"
        ):
            return True
        ...
        for slot, original_type in self.original_inference_results.items():
            if slot in self.evmole_storage_info:
                ...
                if not self._types_compatible(original_type, evmole_type):
                    ...
                    self._get_more_specific_type(evmole_type, original_type)
                    != original_type
                ):
