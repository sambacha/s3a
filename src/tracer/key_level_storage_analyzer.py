...
from .storage_analyzer import StorageLayout, StorageVariable, MappingVariable, ArrayVariable
...
class KeyLevelArrayVariable(ArrayVariable):
    ...
    def __init__(self,
        ...
        self.accesses_by_index: DefaultDict[str, List[StorageAccess]] = defaultdict(list)
...
class KeyLevelStorageAnalyzer(EnhancedStorageAnalyzer):
    ...
    def _extract_base_slot_and_key(self, slot_value: SymbolicValue) -> Tuple[Union[int, str], Optional[str]]:
        """
        ...
        if "keccak256" in expr_str:
            ...
            try:
                ...
                if concrete_slots:
                    base_slot = min(concrete_slots) # Guess the smallest concrete number is the base slot
    ...
    def _analyze_storage_accesses(self, accesses: List[StorageAccess]) -> None:
        ...
        base_slot_accesses: DefaultDict[Union[int, str], List[StorageAccess]] = defaultdict(list)
        access_details: Dict[int, Tuple[Union[int, str], Optional[str]]] = {} # Store details per access pc
        ...
        for base_slot, slot_accesses in base_slot_accesses.items():
            ...
            is_packed = any(hasattr(a, 'mask') and a.mask and a.mask != ((1 << 256) - 1) for a in slot_accesses)
            # TODO: Add struct detection if needed

            if is_mapping:
                self._analyze_key_level_mapping_slot(base_slot, slot_accesses, access_details)
            elif is_array:
                 self._analyze_key_level_array_slot(base_slot, slot_accesses, access_details)
            # elif is_packed:
    ...
    def _analyze_key_level_mapping_slot(self, base_slot: Union[int, str], accesses: List[StorageAccess], access_details: Dict[int, Tuple[Union[int, str], Optional[str]]]) -> None:
        """
        Analyze accesses to a mapping slot at the key level.
        """
        logger.debug(f"Analyzing mapping base slot {base_slot} with {len(accesses)} accesses")
        ...
        for access in accesses:
             ...
             key_repr = details[1] if details and details[1] is not None else "unknown_key"
             # Update call to use renamed method
             var.add_key_access(access, key_repr)
             keys_found.add(key_repr)

        logger.debug(f"Mapping slot {base_slot} accessed with {len(keys_found)} unique keys.")
        self.layout.add_variable(var)


    def _analyze_key_level_array_slot(self, base_slot: Union[int, str], accesses: List[StorageAccess], access_details: Dict[int, Tuple[Union[int, str], Optional[str]]]) -> None:
        """
        Analyze accesses to an array slot at the index level.
        """
        logger.debug(f"Analyzing array base slot {base_slot} with {len(accesses)} accesses")
        ...
        for access in accesses:
             details = access_details.get(access.pc) if access.pc is not None else None
             index_repr = details[1] if details and details[1] is not None else "unknown_index"
             # Update call to use renamed method
             var.add_index_access(access, index_repr)
             indices_found.add(index_repr)

        logger.debug(f"Array slot {base_slot} accessed with {len(indices_found)} unique indices.")
        self.layout.add_variable(var)
