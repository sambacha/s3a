...
class StorageAnalyzer:
    ...
    def __init__(self, max_execution_paths: int = 100) -> None:
        ...
        self.known_patterns = {
            ...
            'erc20': {
                ...
                'allowances': {'slot': 5, 'type': 'mapping(address => mapping(address => uint256))'}
            },
            ...
            'proxy': {
                'implementation': {'slot': '0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc', 'type': 'address'},
                'admin': {'slot': '0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103', 'type': 'address'},
            }
        }
        ...
        for access in accesses:
            ...
            # Detect potential struct patterns (consecutive slots accessed together)
                ...
                for other_access in accesses:
                    ...
                    if isinstance(other_slot, int) and other_slot == normalized_slot + 1:
                        if access.pc and other_access.pc and abs(access.pc - other_access.pc) < 20:
                            struct_slots.add(normalized_slot)
                            struct_slots.add(other_slot)
            
            # Detect packed variables (partial slot operations)
            if hasattr(access, 'mask') and access.mask and access.mask != ((1 << 256) - 1):
    ...
    def _analyze_packed_slot(self, slot: Union[int, str], accesses: List[StorageAccess]) -> None:
        """
    ...
    def _infer_masks_from_values(self, accesses: List[StorageAccess]) -> Dict[int, List[Tuple[int, StorageAccess]]]:
        """
        ...
        if len(accesses) >= 2:
            ...
            if len(concrete_values) >= 2:
                ...
                elif all(v < (1 << 64) for v in concrete_values[:2]) and len(concrete_values) > 2 and concrete_values[2] < (1 << 128):
                    masks = {
                        0: [(64, accesses[0])],
                        64: [(64, accesses[1])],
                        128: [(128, accesses[2])]
                ...
                # Four uint64 values
                elif all(v < (1 << 64) for v in concrete_values) and len(concrete_values) >= 4:
                    masks = {
                        0: [(64, accesses[0])],
                        64: [(64, accesses[1])],
                        128: [(64, accesses[2])],
    ...
    def _infer_type_from_size_and_values(self, size: int, accesses: List[StorageAccess]) -> str:
        """
    ...
    def _analyze_nested_mapping_slot(self, slot: Union[int, str], accesses: List[StorageAccess]) -> None:
        """
        ...
        Args:
            ...
            accesses: List of StorageAccess objects for this slot
        """
        logger.debug(f"Analyzing nested mapping slot {slot} with {len(accesses)} accesses")
    ...
    def _generate_nested_mapping_name(self, key_types: List[str], value_type: str) -> str:
        """
        ...
        for kt in reversed(key_types):
            name_parts.append("By" + kt.capitalize().replace("Uint", "Uint").replace("Int", "Int"))
    ...
    def _detect_known_patterns(self) -> None:
        ...
        if erc20_match:
            ...
            for var in self.layout.variables:
                if isinstance(var, MappingVariable) and var.key_type == "address" and var.value_type == "uint256":
                    balances_found = True
                elif isinstance(var, MappingVariable) and "mapping" in var.var_type and "address" in var.var_type:
    ...
    def _analyze_struct_slot(self, slot: Union[int, str], accesses: List[StorageAccess]) -> None:
        """
        ...
        Args:
            ...
            accesses: List of StorageAccess objects for this slot
        """
        logger.debug(f"Analyzing potential struct slot {slot} with {len(accesses)} accesses")
        ...
        if isinstance(slot, int):
            ...
            if self.layout.get_variable_by_slot(prev_slot) or self.layout.get_variable_by_slot(next_slot):
                is_struct = True
