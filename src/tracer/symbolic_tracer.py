...
class SymbolicValue:
    ...
    def __str__(self) -> str:
        if self.concrete:
            return (
                f"{self.value:#x}" if isinstance(self.value, int) else str(self.value)
            )
        else:
...
class StorageAccess:
    ...
    def __init__(
        ...
        self.function_pc = (
            None  # Program counter of the function containing this access
        )
...
class ExecutionState:
    ...
    def store_memory_region(
        self, offset: int, size: int, values: List[SymbolicValue]
    ) -> None:
...
class SymbolicExecutor:
    ...
    def __init__(
        self, max_paths: int = DEFAULT_MAX_PATHS, max_depth: int = DEFAULT_MAX_DEPTH
    ):
    ...
    def analyze(self, bytecode: str) -> List[StorageAccess]:
        ...
        if direct_accesses:
            logger.info(
                f"Direct analysis found {len(direct_accesses)} storage accesses"
            )
        ...
        logger.info(
            f"Analysis complete: {len(self.storage_accesses)} storage accesses, {self.execution_paths} execution paths"
        )
        return self.storage_accesses

    def _identify_function_entries(
        self, operations: List[Tuple[str, int, Optional[bytes], int]]
    ) -> None:
        """
        ...
        for i, (opcode_name, opcode_value, push_data, offset) in enumerate(operations):
            if opcode_name == "PUSH4" and push_data:
                ...
                if (
                    i + 2 < len(operations)
                    and operations[i + 1][0] == "EQ"
                    and operations[i + 2][0] == "JUMPI"
                ):
                    # Get the jump destination
                    if operations[i + 2][2]:  # JUMPI push_data
                        jump_dest = int.from_bytes(
                            operations[i + 2][2], byteorder="big"
                        )
                        self.function_entries[jump_dest] = f"0x{selector:08x}"
                        logger.debug(
                            f"Identified function entry at PC {jump_dest}: selector {self.function_entries[jump_dest]}"
                        )

    def _direct_storage_analysis(
        self, operations: List[Tuple[str, int, Optional[bytes], int]]
    ) -> List[StorageAccess]:
        """
        ...
        for i, (opcode_name, opcode_value, push_data, offset) in enumerate(operations):
            ...
            elif (opcode_value == Opcode.SHL or opcode_value == Opcode.SHR) and len(
                stack
            ) >= 2:
            ...
            # Handle SLOAD operations
                ...
                if stack:
                    ...
                    logger.debug(
                        f"Direct analysis: SLOAD at PC {offset} for slot {slot}"
                    )
            ...
            # Handle SSTORE operations
                ...
                if len(stack) >= 2:
                    ...
                    accesses.append(access)
                    logger.debug(
                        f"Direct analysis: SSTORE at PC {offset}: {slot} = {value}"
                    )
        ...
        for access in accesses:
            ...
            # Check for mapping access patterns (keccak256 hash)
            elif not access.slot.concrete and isinstance(
                access.slot.value, z3.BitVecRef
            ):
                slot_str = str(access.slot.value)
                if "keccak256" in slot_str:
                    ...
                    if slot_str.count("keccak256") > 1:
                        ...
                        logger.debug(
                            f"Direct analysis: Detected nested mapping access: {access}"
                        )
                    else:
                        logger.debug(
                            f"Direct analysis: Detected mapping access: {access}"
                        )
            ...
            # Check for potential struct slots (consecutive integer slots)
                ...
                for adjacent in [slot_value - 1, slot_value + 1]:
                    if adjacent in self.storage_slots_accessed:
                        ...
                        logger.debug(
                            f"Direct analysis: Detected potential struct at slot {slot_value}"
                        )

        return accesses

    def _find_current_function(
        self, pc: int, operations: List[Tuple[str, int, Optional[bytes], int]]
    ) -> Optional[int]:
        """
    ...
    def _extract_runtime_bytecode(self, bytecode: str) -> Optional[str]:
        ...
        try:
            ...
            for i, (opcode_name, opcode_value, push_data, offset) in enumerate(
                operations
            ):
                if opcode_name == "RETURN" and i > 2:
                    ...
                    if any(op[0] == "CODECOPY" for op in prev_ops):
                        ...
                        for j, (op_name, op_value, op_data, op_offset) in enumerate(
                            prev_ops
                        ):
                            if op_name.startswith("PUSH") and op_data:
                                ...
                                if runtime_start + size * 2 <= len(bytecode):
                                    return bytecode[
                                        runtime_start : runtime_start + size * 2
                                    ]
            ...
            for i, (opcode_name, opcode_value, push_data, offset) in enumerate(
                ...
                if (
                    opcode_name in ("STOP", "RETURN")
                    and offset + 1 < len(bytecode) // 2
                ):
                    # Check if there's more bytecode after this
                    remaining = bytecode[offset * 2 + 2 :]
                    if (
                        len(remaining) > 64
                    ):  # Arbitrary minimum size for runtime bytecode
                        return remaining
        ...
        for access in self.storage_accesses:
            ...
            if not access.slot.concrete and isinstance(access.slot.value, z3.BitVecRef):
                ...
                if "keccak256" in slot_str:
                    ...
                    if hasattr(access.slot, "keccak_args") and access.slot.keccak_args:
                        ...
                        memory_content = access.slot.keccak_args.get(
                            "memory_content", []
                        )
                        if memory_content:
            ...
            # Check for potential struct members (consecutive slots)
                ...
                if slot_value in self.potential_structs:
                    access.is_struct = True
                    logger.debug(
                        f"Detected potential struct member at slot {slot_value}"
                    )

    def _execute(
        self,
        operations: List[Tuple[str, int, Optional[bytes], int]],
        ...
        if self.execution_paths >= self.max_paths:
            logger.warning(
                f"Reached maximum number of execution paths ({self.max_paths})"
            )
            return
    ...
    def _handle_push(
        self, state: ExecutionState, opcode_value: int, push_data: Optional[bytes]
    ) -> None:
    ...
    def _handle_swap(self, state: ExecutionState, opcode_value: int) -> None:
        ...
        if len(state.stack) >= position + 2:
            ...
            state.stack[-1], state.stack[-(position + 2)] = (
                state.stack[-(position + 2)],
                state.stack[-1],
            )
        ...
        if a.concrete and b.concrete:
            ...
            if opcode_value == Opcode.ADD:
                ...
                if a.concrete and b.concrete:
                    ...
                    result.value = z3.simplify(
                        a.value - b.value if hasattr(a.value, "__sub__") else a.value
                    )
                state.push(result)
            elif opcode_value == Opcode.MUL:
                ...
                if b.concrete and b.value == 0:
                    ...
                    if a.concrete and b.concrete:
                        ...
                        result.value = z3.simplify(
                            z3.UDiv(a.value, b.value)
                            if hasattr(a.value, "__truediv__")
                            else a.value
                        )
                    state.push(result)
            elif opcode_value == Opcode.MOD:
                if b.concrete and b.value == 0:
                    ...
                    if a.concrete and b.concrete:
                        ...
                        result.value = z3.simplify(
                            z3.URem(a.value, b.value)
                            if hasattr(a.value, "__mod__")
                            else a.value
                        )
                    state.push(result)
        ...
        if a.concrete and b.concrete:
            if opcode_value == Opcode.SHL:
                ...
                if a.value >= 256:
                    ...
                    if b.value & (
                        1 << 255
                    ):  # Check if b is negative (in 2's complement)
            ...
            if slot.is_keccak:
                ...
                if hasattr(slot, "keccak_args") and slot.keccak_args:
                    ...
                    if memory_content:
                        # Simple heuristic: if first value looks like an address (20 bytes)
                        if any(
                            val.concrete and val.value < (1 << 160)
                            for val in memory_content
                        ):
                            access.key_type = "address"
                        else:
                            access.key_type = "uint256"
