...
def test_simple_addition_bytecode():
    ...
    assert block_0.end_offset == 4, f"Block 0 end offset should be 4, but got {block_0.end_offset}"

    expected_opcodes = ["PUSH1", "PUSH1", "ADD"]
    actual_opcodes = [instr.opcode for instr in block_0.instructions]
    assert len(actual_opcodes) == len(expected_opcodes), f"Block 0 should have {len(expected_opcodes)} instructions"
    assert actual_opcodes == expected_opcodes, f"Block 0 opcodes mismatch: Expected {expected_opcodes}, Got {actual_opcodes}"

    # Check stack analysis (if available)
    # analyze_stack_locally should have been called in __init__
    assert hasattr(block_0, 'stack_effect'), "Block 0 should have stack_effect attribute"
    ...
    assert block_0.stack_effect == expected_stack_effect, f"Block 0 stack effect mismatch: Expected {expected_stack_effect}, Got {block_0.stack_effect}"
