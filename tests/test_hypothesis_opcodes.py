...
opcode_bytes_strategy = st.integers(min_value=0x00, max_value=0xFF).map(
    lambda i: i.to_bytes(1, "big")
)
...
)  # Increase examples, disable deadline for potentially slow disassembly
...
def test_disassemble_does_not_crash_random_bytes(bytecode_hex: str):
    ...
    try:
        ...
        pytest.fail(
            f"disassemble_bytecode raised unexpected exception {type(e).__name__}: {e} on input {bytecode_hex}"
        )
...
def test_disassemble_valid_sequences(bytecode_hex: str):
    ...
    try:
        ...
        pytest.fail(
            f"disassemble_bytecode raised unexpected exception {type(e).__name__}: {e} on valid input sequence {bytecode_hex}"
        )
...
def test_disassemble_single_push_operation(push_info):
    ...
    try:
        ...
        assert op_val == push_opcode_val, (
            "Opcode value should match generated PUSH opcode"
        )
        assert op_name == f"PUSH{len(push_data_bytes)}", (
            "Opcode name should match PUSH size"
        )
        assert op_data == push_data_bytes, "Push data bytes should match generated data"

    except Exception as e:
        pytest.fail(
            f"disassemble_bytecode failed on PUSH operation {bytecode_hex}: {e}"
        )
...
@settings(
    max_examples=len(single_byte_opcodes), deadline=None
)  # Test all defined single opcodes
    ...
    for i in range(length):
        if i == invalid_pos:
            ...
            if Opcode.PUSH1 <= opcode_val <= Opcode.PUSH32:
                ...
                opcode_val in range(0x0C, 0x10)
                or opcode_val in range(0x21, 0x30)
                or opcode_val in range(0x49, 0x50)
                or opcode_val in range(0xA5, 0xB0)
                or opcode_val in range(0xB3, 0xC0)
                or opcode_val in range(0xF6, 0xFA)
                or opcode_val == 0xFC
                or opcode_val == 0xFE
            ):
...
def test_disassemble_handles_invalid_opcodes(data):
    ...
    try:
        ...
        for op_name, op_val, op_data, op_offset in operations:
            ...
            if is_known_invalid:
                ...
                assert op_name == expected_name, (
                    f"Opcode {op_val:#x} at offset {op_offset} should be marked {expected_name}, but got {op_name}"
                )
                found_invalid = True
    ...
    # This assertion might be too strict if the invalid byte causes parsing to stop early
        ...
        pytest.fail(
            f"disassemble_bytecode failed unexpectedly on sequence with invalid opcodes {bytecode_hex}: {e}"
        )
